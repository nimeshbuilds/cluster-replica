...
> **Experimental alpha:** this guide uses the published `v0.2.0-alpha.1` CLI and operator image with the repository’s disposable fixtures. Replicove installs vCluster for you when you approve the replica; you do not need an existing vCluster.
...
export REPLICOVE_RELEASE=v0.2.0-alpha.1
...
You stay on the normal `main` branch. The CLI reports `v0.2.0-alpha.1`; the operator image is pulled automatically from GHCR. The Kubernetes and vCluster versions used by the demo remain pinned in the repository.
...
rk install --image ghcr.io/nimeshbuilds/replicove:0.2.0-alpha.1 --values test/e2e/replicove-values.yaml
...
| Operator `ImagePullBackOff` | Check node access to `ghcr.io` and confirm the image is `ghcr.io/nimeshbuilds/replicove:0.2.0-alpha.1`. |
