...
```bash
export REPLICOVE_EXAMPLES=https://raw.githubusercontent.com/nimeshbuilds/replicove/v0.2.0-alpha.1/examples/yaml
...
To connect locally, download and extract your platform’s `replicove` CLI archive from the [alpha release](https://github.com/nimeshbuilds/replicove/releases/tag/v0.2.0-alpha.1) into your working directory. Keep this command running in the same host context:
...
helm show values oci://ghcr.io/nimeshbuilds/charts/replicove \
  --version 0.2.0-alpha.1 > operator-values.yaml
```
...
After **all** replicas and access requests managed by the operator have finished cleanup:
