...
The overview shows replica counts, readiness, requests needing attention and active chaos experiments. Each replica includes a lifecycle display, expiry, pinned runtime versions, metadata-only plan and dependency evidence, and a copyable CLI inspection command. Mirror and experiment panels show phases and expiry. It does not display captured resource bodies, request specs, Secret contents, database rows or credentials, and it has no write endpoints.
