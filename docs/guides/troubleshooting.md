...
### A PVC waits for a guest test Pod

Check for guest Pods that still reference the copied claim, including completed probe Pods left by integration tests. Published **v0.3.0-alpha.1** has a cleanup-ordering defect in this case: it can wait for the PVC before deleting the replica-owned namespace containing its extra consumer. **Upgrade to v0.3.0-alpha.2**, whose fix passed exact-source and published-artifact checks; see the [versioned validation record](../validation.md#alpha2-release-qualification).

In an exclusively owned guest namespace, alpha.2 requests namespace deletion while that ordinary PVC remains pending, then wait for normal Kubernetes cleanup and the recorded volume checks. Deprovisioning includes guest-created workloads inside the disposable namespace. It still preserves ownership checks, finalizers and controller/custom-resource ordering. See [guest-created volume consumers](cleanup.md#guest-created-volume-consumers).

In any pre-existing guest namespace reused without adoption, the namespace and foreign Pods are preserved during guest-resource cleanup. This includes `default` in a managed guest as well as borrowed existing-target namespaces. Its administrator must remove or reconfigure the exact foreign consumer before the owned PVC can finish deleting. Do not delete the shared namespace as a workaround. On alpha.1, an administrator can similarly remove the intended disposable test Pod through an already authorized guest route and let cleanup retry. New Replicove access sessions are unavailable once cleanup starts; use the administrator's existing route rather than expanding requester permissions. Never remove PVC protection, protected state or Replicove finalizers to hide this wait.
