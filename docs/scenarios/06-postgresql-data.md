...
1. Install Replicove with the database module disabled. Seed a PostgreSQL source and an application. The source database has two customers and their orders in tenants `10` and `20`, with an email-based foreign key. A dedicated `replica_reader` role receives read privileges; an attempted source update with that role must fail.
2. Create an explicit database grant. It authorizes this one source database, credentials in the protected namespace, TLS `require`, the pinned PostgreSQL image, storage/bounds, tenant `10` filters and matching token-mask domains for customer/order email fields. The replica selects the application, not the source database Deployment, and patches its connection to generated target credentials.
...
5. Query the copied database and application. Exactly the selected tenant's joined customer/order remains, both equality-linked email fields have the same 64-character token, and the foreign key is validated. The application resolves and reaches the generated database Service. The original fixture marker must be absent from the final PostgreSQL data directory, including its WAL; a missing directory or failed read is an error, not a clean result.
...
Read the [fixture](https://github.com/nimeshbuilds/replicove/blob/main/hack/e2e-database.sh), [synthetic schema/source](https://github.com/nimeshbuilds/replicove/blob/main/test/database/source.yaml), [grant](https://github.com/nimeshbuilds/replicove/blob/main/test/database/grant.yaml), [application wiring](https://github.com/nimeshbuilds/replicove/blob/main/test/database/replication.yaml) and [installation values](https://github.com/nimeshbuilds/replicove/blob/main/test/database/values.yaml). The [PostgreSQL guide](../guides/postgresql.md) explains `Token`, `Null` and `Constant` strategies, approved subsets, native/declared relationships and storage sizing. This live fixture uses domain-linked `Token` masks; it does not claim every mask/schema combination is represented by these rows.
