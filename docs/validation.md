...
The planned **v0.3.0-alpha.2** fix permits progress past a pending ordinary PVC only when its guest namespace is also inventoried as owned. Pre-existing guest namespaces reused without adoption and their foreign consumers remain protected, ownership conflicts remain blocking, and custom-resource/controller dependencies retain their cleanup order. These behavior and regression tests are still awaiting source, exact-main and published-artifact qualification. The ten-scenario collection must subsequently pass all 16 documented variants using the newly published pinned release before being reported as verified.
