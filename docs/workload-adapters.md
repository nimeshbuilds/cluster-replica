...
Replicove's portable adapter uses CRDs, explicit Helm revision capture, dependency ordering, value overrides, and custom readiness checks. Workload-specific settings stay in inspectable fixture YAML. The cert-manager, Spark and native policy profiles passed source-to-guest functional checks and owned cleanup at `fa2889c`; see the [recorded evidence](validation.md). Trino returned the expected query result but failed tunnel teardown in that run. Later commits must pass the same jobs; these small fixtures do not certify production scale.
...
| Kubernetes admission policy | Native `admissionregistration.k8s.io/v1` | Restrict the fixture to named test probes | Guest API denies the unlabelled probe and accepts the labelled one |
...
`test/workloads/charts` vendors the small, unmodified Apache-2.0 chart inputs with their licenses to avoid CDN access failures in CI. `hack/fetch-workload-charts.py` verifies and pins the chart archives by SHA-256 from the official chart index or GitHub release asset digest. `hack/e2e-workload.sh NAME` installs the source release, captures its chart/values and selected custom resources, creates a new vCluster, checks real guest behavior, and verifies owned cleanup. It preserves source resources and uploads only sanitized evidence.
