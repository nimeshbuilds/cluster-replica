...
guest_inventory() {
  result=$?
  trap - EXIT
  kubectl get pods,services,jobs,deployments --all-namespaces -o json \
    | python3 "$E2E_REPO/test/e2e/inventory.py" > "$E2E_RESULTS/guest-$mode.json" || true
  exit "$result"
}
trap guest_inventory EXIT
kubectl get --raw /readyz
kubectl version --output=json > "$E2E_RESULTS/guest-version.json"
python3 - "$E2E_RESULTS/guest-version.json" <<'PY'
import json,sys
assert json.load(open(sys.argv[1]))['serverVersion']['gitVersion'] == 'v1.36.0'
PY
if [[ "$mode" == create ]]; then
  kubectl create namespace smoke
  # kubectl's trailing arguments replace the image entrypoint, so include the
  # executable as well as its netexec subcommand.
  kubectl -n smoke create deployment smoke --image=registry.k8s.io/e2e-test-images/agnhost:2.63.0 -- /agnhost netexec --http-port=8080
